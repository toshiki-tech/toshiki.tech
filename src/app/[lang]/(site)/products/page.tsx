            ...
            <div key={product.id} className={`flex flex-col ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} gap-12 md:gap-20 items-center`}>
              <div className="flex-1 w-full order-1">
                <div className="aspect-[16/10] bg-[var(--muted)] rounded-2xl border border-[var(--border)] relative overflow-hidden group shadow-2xl transition-all hover:-translate-y-1 duration-500">
                  <div className="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-[var(--muted)]/50 to-transparent z-10 pointer-events-none" />
                  {product.imageUrl ? (
                    <ZoomableImage
                      src={product.imageUrl}
                      alt={t.title}
                      className="w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-700"
                      zoomLabel={ZOOM_LABELS[lang].zoom}
                      closeLabel={ZOOM_LABELS[lang].close}
                    />
                  ) : (
              ...
              <div className="flex-1 w-full space-y-6">
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  {product.downloads && (
                    <Link href={`/${lang}/p/${product.slug}#download`} className="btn-secondary">
                      {lang === 'en' ? 'Download' : lang === 'zh' ? '下载' : lang === 'zh-tw' ? '下載' : 'ダウンロード'}
                    </Link>
                  )}
